...
MIN_FRAME_GAP = 5         # Minimum gap between selected frames to reduce temporal redundancy
TRAIN_SIZE = 1000         # Target number of frames for the training set
VAL_SIZE = 150            # Target number of frames for the validation set
TEST_SIZE = 200           # Target number of frames for the test set
RANDOM_SEED = 42          # Seed for reproducibility

# Selection Weights
RARE_CLASS_WEIGHT = 3     # Score multiplier to prioritize frames with less common classes
...
# These classes increase the sampling score of a frame
RARE_CLASSES = {
...
def extract_frame_idx(filename: str) -> int:
    ...
    # Return the last sequence of digits found (robust against numbers in path)
...
def select_training_candidates(all_images: List[str]) -> Tuple[List[str], Set[str]]:
    ...
    Selection Criteria:
    1. Frame must contain both 'Pedestrian' and 'Biker' classes (Interaction scenes).
    2. Frames must be separated by at least MIN_FRAME_GAP (Temporal diversity).
    ...
    for img_path in all_images:
        # Resolve label path based on image filename
        base_name = os.path.splitext(os.path.basename(img_path))[0]
        label_path = os.path.join(LABELS_DIR, base_name + ".txt")
        
        # Check class content within the frame
        classes = get_label_classes(label_path)
        if not classes:
            continue

        # Filter: Ensure core interaction classes exist
        if not (PED_ID in classes and BIKER_ID in classes):
            continue

        # Filter: Enforce temporal gap to prevent overfitting on nearly identical frames
        ...
        # Base score = total number of objects
        # Bonus score = extra points for rare classes
        score = len(classes)
    ...
    # Rank candidates by score (descending) and select top K
...
def select_random_subset(pool: List[str], excluded: Set[str], count: int) -> List[str]:
    ...
    Args:
        ...
        excluded (Set[str]): Images to exclude from selection (e.g., already in train set).
...
def main():
    ...
    # Aggregate all valid image files and sort them sequentially
    all_images = [
    ...
    # 1. Generate Training Split (Heuristic selection)
    train_imgs, train_set = select_training_candidates(all_images)
    
    # 2. Generate Validation Split (Random subset excluding training data)
    val_imgs = select_random_subset(all_images, train_set, VAL_SIZE)
    
    # 3. Generate Test Split (Random subset excluding train and val data)
    excluded_from_test = train_set.union(set(val_imgs))
    test_imgs = select_random_subset(all_images, excluded_from_test, TEST_SIZE)

    # Convert paths to relative format for portability and write .txt split files
    ...
    # 4. Generate YOLO Data Configuration (data.yaml)
    data_config = {
        ...
        "names": {v: k for k, v in utils.CLASS_MAP.items()}, # Class ID to Name mapping
        "nc": len(utils.CLASS_MAP) # Number of classes
    }
