...
2. Geometry, Math, and NMS helpers.
3. Visualization tools.
4. Core Tracking Logic (Kalman Filter, Track, ByteTrackLike).
5. Evaluation Metrics (HOTA, MSE).
"""
...
AREA_W_LAST = 0.3
TRAJ_MAX_LEN = 2000
MISS_FRAMES_TO_DROP_PATH = 100

# --- Border Logic ---
BORDER_RATIO = 0.1
BORDER_REVIVE_PENALTY = 2.0
ANGLE_CHANGE_THRES = 45.0
PREFER_NEW_AT_BORDER = True
...
def is_near_border(bbox: np.ndarray, frame_shape: Tuple[int, int], border_ratio: float = 0.04) -> bool:
    """Checks if a box is within the border margin of the frame."""
    x1, y1, x2, y2 = bbox
    H, W = frame_shape[:2]
...
def angle_to_compass(deg: float) -> str:
    """Converts degrees to 8-point compass direction string."""
...
def draw_box_with_id(frame: np.ndarray, bbox: np.ndarray, cls: int, tid: int, 
    ...
    """Draws a bounding box with Class Name, ID, and Confidence."""
...
def draw_trajectory(frame: np.ndarray, pts: List[Tuple[int, int]], color: Tuple[int, int, int] = (255, 255, 255)) -> None:
    """Draws the track history as a polyline."""
    if len(pts) < 2: return
...
def draw_velocity_arrow(frame: np.ndarray, cx: float, cy: float, deg: float, 
    ...
    """Draws an arrow indicating motion direction."""
...
def create_kalman_filter(initial_cxcywh: np.ndarray) -> Any:
    """Initializes Kalman Filter or dummy predictor."""
    if HAS_FILTERPY:
